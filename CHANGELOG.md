...
### Frameworkless renderer + 24-block catalogue + runtime discovery

v0.4.0 shipped the typed-CRUD database model but kept the Lit-based
renderer + a 15-section catalogue baked into the skills. v0.5.0
rewrites the renderer end-to-end and re-tools the agent surface
around runtime block discovery.

**Renderer (server-side):**

- Lit is gone. `src/render.ts` is now ~370 LOC of frameworkless SSR:
  reads `tokens.css` + `primitives.css` once at module init, dispatches
  every `Page.sections[]` entry through a typed block registry, inlines
  per-block CSS only when the block appears on a page.
- 24 catalogue blocks ship: hero variants (centered, split-image, stats,
  minimal), pricing (cards-3, simple), features (feature-grid, grid-6),
  testimonials (grid-3, quote-big), cta (centered, banner), faq, stats
  (row-4, with-context), logos-row, nav-simple, footer-simple, team-grid,
  steps-numbered, text, plus interactive blocks (announcement-bar,
  cookie-banner, newsletter-form).
- Each interactive block ships a `client.ts` bundled via `Bun.build` to
  `/public/_h/<block>.js`; the page renderer emits a deduped
  `<script type="module">` per block on the page.
- LCP candidate auto-marking: first `ctx.lcpCandidate(src)` call wins;
  the renderer post-processes that `<img>` to `loading="eager"` +
  `fetchpriority="high"`.
- Typed JSON-LD: blocks emit `Contribution[]` (faq-item, product-offer,
  review, employee, primary-image, …) and the page-level `assembleGraph()`
  merges them into one schema.org @graph with Organization + WebSite +
  WebPage + per-page nodes. Multiple FAQ blocks now merge into ONE valid
  FAQPage node (was emitting N invalid nodes in v0.4).

**MCP tool surface (24 tools, up from 21):**

- `list_blocks` — every block type with title + summary + whenToUse.
- `get_block(type)` — full JSON Schema (Draft 2020-12) + canonical
  examples + defaults for one block. The schema is the authoring
  source of truth.
- `validate_block(type, data)` — returns `{ok, errors[{path, expected,
  got, hint}]}`. Tailored hints for `unrecognized_keys` (".strict() —
  remove the unknown key"), `invalid_type`, `too_small`, `too_big`,
  `invalid_value`. The agent's self-correction loop reads these.

**Preview URL surfacing (every content-mutating tool):**

`upsert_page`, `upsert_post`, `update_site_config`, `publish_site`,
and `discard_changes` now embed `previewUrl` + `liveUrl` in their
response. The preview URL renders from drafts and auto-reloads via
SSE — open it once, keep iterating.

**Plugin (agent-facing) changes:**

- `section-catalogue` skill rewritten around MCP discovery. Old 15-name
  inventory removed; the agent learns the catalogue from `list_blocks`
  + `get_block` at runtime. Composition recipes updated with current
  canonical block names. One canonical multi-section example replaces
  the 15 stale single-block examples.
- New `/blocks` slash command (exposes `list_blocks`) for ad-hoc
  discovery.
- `edit-page`, `edit-post`, `update-theme` skills updated to (a) call
  MCP `validate_block` before upsert, (b) surface the preview URL on
  every successful write.
- `site-author` + `content-validator` agents rewired around the MCP
  discovery + validation tools.
- `custom-components` skill removed entirely (Lit-based feature deleted
  in this release).

**Removed:**

- `lit` + `@lit-labs/ssr` dependencies.
- `src/components/` (17 Lit components).
- `src/custom-sections.ts` + the customer-authored Lit-component pipeline.
  Custom block authoring will return via a registry extension API in a
  future release.

**Schema artifact:**

- `plugin/shared/block-schemas.json` is regenerated against the live
  registry on every release. The plugin pre-flight validator + MCP
  tools both read it.
