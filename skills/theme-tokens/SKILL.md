...
1. **Preset themes** (`site_config.theme: "default" | "editorial"`) — full token bundles maintained by the platform.
2. **`theme_overrides`** in `site_config` — per-site CSS-var overrides on top of the preset. Bounded to an 8-token allow-list; everything outside is silently dropped.

You set both via `update_site_config({site, patch})` — the `/update-theme` action skill is the wrapper.
...
That's the shape of the patch you pass to `update_site_config`. No `custom_css` field. The preset gives you the whole token bundle; overrides tweak the 8 highest-leverage tokens.
...
Use the **`theme-tokens` examples** dir to see both presets with and without overrides applied. The example files contain `site_config` objects in the same shape you pass to `update_site_config`.
...
The presets bring their own webfonts (editorial preloads Spectral via Google Fonts). For overrides that point at a custom face, **load the font via a `<link>` you embed using a `raw-embed` section** — the override JSON itself can't add `<link>` tags.

Two-step recipe:

1. Add a `raw-embed` section near the top of the page (via `upsert_page`) with the font preload:
...
2. Set the override in site_config:

   ```json
   { "theme_overrides": { "--font-sans": "'Geist Sans', system-ui, sans-serif" } }
   ```

Better long-term: self-hosted fonts + the preload added via the platform's chrome — coming in a future milestone.
...
The bundled validator checks a `site_config` patch (or just a `theme_overrides` object) against the allow-list:

```bash
node ${CLAUDE_PLUGIN_ROOT}/skills/theme-tokens/scripts/validate-theme-overrides.js path/to/site-config.json
```
...
- **`examples/site.json.default.json`** — minimal site_config on the default preset, no overrides.
